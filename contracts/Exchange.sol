...
contract Exchange is GovernanceOwnable, AccessControl, ReentrancyGuard {
  ...
  modifier onlyAuctionOwner(uint256 _auctionId) {
    require(msg.sender == exchangeStorage.getAuction(_auctionId).owner, "Exchange: Caller is not the auction owner");
    _;
  }
  ...
  function createAuction(
    ...
    // validation steps
    require(classIds.length == nonceIds.length && classIds.length == amounts.length, "Exchange: inputs not correct");
    ...
    require(minDBITAmount < maxDBITAmount, "Exchange: min DBIT Amount Should be less than max currency amount");
    ...
    exchangeStorage.createAuction(creator, block.timestamp, auctionDuration, DBITAddress, maxDBITAmount, minDBITAmount);
    ...
    IERC3475(erc3475Address).transferFrom(creator, exchangeStorageAddress, transactions);
  ...
  function bid(uint256 _auctionId) external nonReentrant {
    ...
    require(auction.auctionState == IExchangeStorage.AuctionState.Started, "bid is completed already");
    ...
    IERC20(auction.erc20Currency).safeTransferFrom(msg.sender, auction.owner, finalPrice);
  ...
  function getAuction(uint256 _auctionId) external view returns (IExchangeStorage.AuctionParam memory) {
